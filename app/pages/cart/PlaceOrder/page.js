...
const handleChange = (e, field) => {
  ...
  const validateForm = () => {
    ...
    if (!data.addressLine1) newErrors.addressLine1 = "Address Line 1 is required";
  ...
  useEffect(() => {
    ...
    const fetchCart = async () => {
      try {
        const response = await fetch(`${API_URL}/CartItems/ByUser/${userId}`);
          ...
          <form className="placeOrderForm" onSubmit={handleSubmit}>
            ...
            <div className="formGroup">
              ...
              <input
                type="text"
                name="addressLine2"
                className={`inputField ${errors.addressLine2 ? "inputError" : ""}`}
                  ...
                  {cartItems.map((item, index) => (
                    <tr key={index}>
                      <td className="place-image-column">
                        {item.imageUrl ? (
                          <img src={item.imageUrl} alt={`Product ${item.skuID_Master}`} />
                        ) : (
